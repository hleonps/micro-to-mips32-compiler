...
// Verifies the list of ids for "read"
void id_list()
{
	/* <id list> ::= ID { , ID} */
	match(ID);
...
void expression(expr_rec *target, int cond_flag)
{
	...
	// If target is NULL is because it wants to be printed
	if (target == NULL)
	{
		char *tmp = get_temp();
	...
	// Verifies if it is part of a conditional expression.
	if (!cond_flag)
	{
		conditional(target, t);
	}
...
// Gets the operation of an expression.
op_rec add_op()
{
	token tok = next_token();
	op_rec op;
...
// Verifies if the grammar token is the same to the program
void match(token t)
{
	if (tmp_token == NULL)
	{
